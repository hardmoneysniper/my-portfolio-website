...
TEST(test_simple_player_make_trump2) {
	...
	Card six_clubs = Card(Card::RANK_SIX, Card::SUIT_CLUBS);
...
TEST(whole2) {
	...
	//two->add_and_discard(ten_hearts);
	...
	nine->add_and_discard(ten_hearts);

	card_led = nine->lead_card(Card::SUIT_SPADES);
	ASSERT_EQUAL(card_led, ace_clubs);

	card_play = eight->play_card(ace_clubs, Card::SUIT_SPADES);
	...
	ASSERT_TRUE(orderup);
	ASSERT_EQUAL(trump, Card::SUIT_CLUBS);
	three->add_and_discard(king_diamonds);

	card_led = three->lead_card(Card::SUIT_CLUBS);
	ASSERT_EQUAL(card_led, ace_diamonds);

	delete three;

	//right bower in second round
	...
	card_led = player1->lead_card(Card::SUIT_DIAMONDS);
	...
	//1:only trump card and 2:without lead suit
	...
	ASSERT_TRUE(orderup);//ASSERT_FALSE(orderup1);
	//ASSERT_EQUAL(trump, Card::SUIT_DIAMONDS);
	/*
	six->add_card(ten_spades);
	six->add_card(queen_spades);
